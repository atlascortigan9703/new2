...
function displaySites(sites) {
    ...
    sites.forEach(site => {
        ...
        siteCard.addEventListener('click', () => openModal(site));
...
function toggleVoiceNarration() {
    ...
    if (isVoiceEnabled) {
        voiceToggle.innerHTML = '<i class="fas fa-volume-mute"></i><span>Voice Narration</span>';
        voiceToggle.style.backgroundColor = '#e74c3c';
        
        // If modal is open, start reading the current site
        if (modal.style.display === 'block') {
            const siteName = document.querySelector('.modal-details h2').textContent;
        ...
        voiceToggle.innerHTML = '<i class="fas fa-volume-up"></i><span>Voice Narration</span>';
        voiceToggle.style.backgroundColor = '#3498db';
        stopSpeech();
    }
}

// Read site description using Web Speech API
function readSiteDescription(site) {
    stopSpeech();
    
    if ('speechSynthesis' in window) {
        const speech = new SpeechSynthesisUtterance();
        speech.text = `${site.name}. ${site.description}`;
        speech.rate = 0.9;
        speech.pitch = 1;
        speech.volume = 1;
